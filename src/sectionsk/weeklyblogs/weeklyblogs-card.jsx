...
export default function PostCard({ platform, content, index, isGen }) {
  ...
  useEffect(() => {
    ...
    const imgMatch = content.match(/<image[^>]+src="(.*?)"[^>]*>/i);
  ...
  return (
    <Grid xs={12} sm={latestPostLarge ? 12 : 6} md={latestPostLarge ? 3 : 4}>
      ...
      <Card sx = {{ height: '397.5px', borderRadius: '8px', border: '2.5px solid', borderColor: 'darkred', backgroundColor: 'white', cursor: 'pointer',
